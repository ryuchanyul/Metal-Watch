...
// Kitco에서 로듐(Rh) spot 가격 fetch. KOMIS/LME/LBMA에 없는 PGM 대체 소스.
// HTML 안 JSON 구조: "symbol":"RH"..."results":[{...,"mid":8325,...,"unit":"OUNCE"}]
async function fetchKitcoRhodium() {
  try {
    const res = await fetch("https://www.kitco.com/charts/rhodium", {
      headers: FETCH_HEADERS
    });
    if (!res.ok) return null;
    const html = await res.text();
    const m = html.match(/"symbol":"RH"[\s\S]{0,500}?"mid":([\d.]+)/);
    if (!m) return null;
    const rawOz = parseNumber(m[1]);
    if (!Number.isFinite(rawOz) || rawOz <= 0) return null;
    return {
      name: "Rhodium",
    ...
    console.warn("[kitco] Rhodium fetch 실패:", e.message);
    return null;
  }
}
...
export async function fetchPricesFromSources() {
  ...
  const [commoditiesRes, currenciesRes, naverUsdKrw, rhodium] = await Promise.all([
  ...
  // Kitco 로듐 병합 (TE에 없으므로 별도 fetch)
  if (rhodium) prices.Rh = rhodium;
